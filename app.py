...
def ingreso_inventario_pantalla(local_id, user_key):
    ...
    if sel:
        ...
        with c2:
            ...
            cant = placeholder.number_input("Cantidad:", min_value=0.0, value=float(st.session_state.resultado_calc), key=f"c_{st.session_state.resultado_calc}")
        with c3:
        ...
        if st.button("Añadir a la lista"):
            st.session_state.carritos[user_key].append({"id_producto": p['id'], "Producto": p['nombre'], "Ubicación": ubi, "Cantidad": float(cant), "Formato": p['formato_medida'], "Factor": extraer_valor_formato(p['formato_medida'])})
    ...
    if st.session_state.carritos[user_key]:
        ...
        if st.button("🚀 FINALIZAR"):
            for r in ed.to_dict(orient='records'):
                supabase.table("movimientos_inventario").insert({"id_local": local_id, "id_producto": r['id_producto'], "cantidad": r['Cantidad']*r['Factor'], "tipo_movimiento": "AJUSTE", "ubicacion": r['Ubicación']}).execute()
...
def main():
    ...
    if st.sidebar.button("📋 Ingreso"): st.session_state.opt = "📋 Ingreso"; st.rerun()
    if st.sidebar.button("📊 Reportes"): st.session_state.opt = "📊 Reportes"; st.rerun()
    if user['role'] == "Admin":
        if st.sidebar.button("⚙️ Maestro"): st.session_state.opt = "⚙️ Maestro"; st.rerun()
